...
class CampTix_Payment_Method_Knit_Pay_Connect extends CampTix_Payment_Method {
	...
	function payment_settings_fields() {
	    $this->add_settings_field_helper( 'server_rest_url', __( 'Knit Pay Server Rest URL', 'campt-indian-payment-gateway' ) , array( $this, 'field_text' ) );
	    $this->add_settings_field_helper( 'server_username', __( 'Server Username', 'campt-indian-payment-gateway' ) , array( $this, 'field_text' ) );
	    $this->add_settings_field_helper( 'server_password', __( 'Server Application Password', 'campt-indian-payment-gateway' ) , array( $this, 'field_text' ) );
	    $this->add_settings_field_helper( 'title', __( 'Title', 'campt-indian-payment-gateway' ) , array( $this, 'field_text' ) );
	    $this->add_settings_field_helper( 'config_id', __( 'Configuration ID', 'campt-indian-payment-gateway' ), array( $this, 'field_text' ) );
	}
	...
	function payment_return() {
		...
		$response = wp_remote_get( $this->options['server_rest_url'] . 'knit-pay/v1/payments/' . $knit_pay_payment_id, array(
	...
	public function payment_checkout( $payment_token ) {
		...
		$response = wp_remote_post( $this->options['server_rest_url'] . 'knit-pay/v1/payments/', array(
		    'headers' => $this->get_request_headers(),
	...
	private function handle_checkout_error($payment_token, $attendee_id, $error_message){
	    ...
	    if ( ! empty( $error_message ) ) {
	        $camptix->log( 'An error occurred in the Knit Pay connection. ' . esc_html( $error_message ), $attendee_id, array( 'payment_token' => $payment_token ), 'knit-pay-connect' );
